...
for i = 2,3 do
  ...
  SET_SPELL_ENTRY(availableNums[i-1], 4, M_SPELL_GHOST_ARMY, 20000, 64, 3, 0)
    ...
    if (_gsi.Counts.GameTurn > tickCyanAttack) then
      ...
      if (_gsi.Players[TRIBE_BLUE].NumPeople > 0 or _gsi.Players[TRIBE_RED].NumPeople > 0) then
        if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_CYAN, M_PERSON_RELIGIOUS) > 4) then
          ...
          ATTACK(TRIBE_CYAN, wta, 4+G_RANDOM(7), ATTACK_BUILDING, T_MODEL_NONE, 850, M_SPELL_INVISIBILITY, M_SPELL_SWAMP, M_SPELL_SWAMP, ATTACK_NORMAL, 0, mkr, NO_MARKER, NO_MARKER)
        end
      else
        if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_CYAN, M_PERSON_RELIGIOUS) > 4) then
          if (_gsi.Players[TRIBE_GREEN].NumPeople > 0) then
            ...
            WRITE_CP_ATTRIB(TRIBE_CYAN, ATTR_AWAY_WARRIOR, 20+G_RANDOM(80))
            WRITE_CP_ATTRIB(TRIBE_CYAN, ATTR_AWAY_MEDICINE_MAN, 100)
            ATTACK(TRIBE_CYAN, TRIBE_GREEN, 12+G_RANDOM(14), ATTACK_BUILDING, T_MODEL_NONE, 850, M_SPELL_INVISIBILITY, M_SPELL_SWAMP, M_SPELL_SWAMP, ATTACK_NORMAL, 0, mkr, NO_MARKER, NO_MARKER)
          end
        end
      end
    elseif (_gsi.Counts.GameTurn > tickOrangeAttack) then
      ...
      if (G_RANDOM(100) > 30) then
        if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_ORANGE, M_PERSON_SUPER_WARRIOR) > 4) then
          ...
          ATTACK(TRIBE_ORANGE, wta, 4+G_RANDOM(12), ATTACK_BUILDING, T_MODEL_NONE, 850, M_SPELL_INVISIBILITY, M_SPELL_LIGHTNING_BOLT, M_SPELL_LIGHTNING_BOLT, ATTACK_NORMAL, 0, 28, NO_MARKER, NO_MARKER)
        end
      else
        if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_ORANGE, M_PERSON_SUPER_WARRIOR) > 4) then
          if (_gsi.Players[TRIBE_CYAN].NumPeople > 0) then
            ...
            WRITE_CP_ATTRIB(TRIBE_ORANGE, ATTR_AWAY_WARRIOR, 20+G_RANDOM(80))
            WRITE_CP_ATTRIB(TRIBE_ORANGE, ATTR_AWAY_MEDICINE_MAN, 100)
            ATTACK(TRIBE_ORANGE, TRIBE_CYAN, 6+G_RANDOM(14), ATTACK_BUILDING, T_MODEL_NONE, 850, M_SPELL_INVISIBILITY, M_SPELL_LIGHTNING_BOLT, M_SPELL_LIGHTNING_BOLT, ATTACK_NORMAL, 0, 29, NO_MARKER, NO_MARKER)
          end
        end
      end
    end
    ...
    if (_gsi.Counts.GameTurn > tickCyanBoat) then
      ...
      if (PLAYERS_VEHICLE_OF_TYPE(TRIBE_CYAN, M_VEHICLE_BOAT_1) > 0 and FREE_ENTRIES(TRIBE_CYAN) > 1) then
        if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_CYAN, M_PERSON_RELIGIOUS) > 3) then
          ...
          ATTACK(TRIBE_CYAN, wta, 4+G_RANDOM(4)*4, ATTACK_BUILDING, T_MODEL_NONE, 850, M_SPELL_INVISIBILITY, M_SPELL_SWAMP, M_SPELL_SWAMP, ATTACK_BY_BOAT, 0, NO_MARKER, NO_MARKER, NO_MARKER)
        end
      end
    elseif (_gsi.Counts.GameTurn > tickOrangeBoat) then
      ...
      if (PLAYERS_VEHICLE_OF_TYPE(TRIBE_ORANGE, M_VEHICLE_BOAT_1) > 0 and FREE_ENTRIES(TRIBE_ORANGE) > 1) then
        if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_ORANGE, M_PERSON_RELIGIOUS) > 3) then
          ...
          ATTACK(TRIBE_ORANGE, wta, 4+G_RANDOM(4)*4, ATTACK_BUILDING, T_MODEL_NONE, 850, M_SPELL_INVISIBILITY, M_SPELL_LIGHTNING_BOLT, M_SPELL_LIGHTNING_BOLT, ATTACK_BY_BOAT, 0, NO_MARKER, NO_MARKER, NO_MARKER)
        end
      end
    end
