...
stone_head_pos = MAP_XZ_2_WORLD_XYZ(64, 190)
lite_attack = {
...
conv_center_pos = {
MAP_XZ_2_WORLD_XYZ(140, 120),
MAP_XZ_2_WORLD_XYZ(0, 226),
MAP_XZ_2_WORLD_XYZ(146, 230)
}
...
SHAMAN_DEFEND(TRIBE_CYAN, 140, 98, TRUE)
SHAMAN_DEFEND(TRIBE_BLACK, 252, 222, TRUE)
SHAMAN_DEFEND(TRIBE_ORANGE, 150, 232, TRUE)
SET_DRUM_TOWER_POS(TRIBE_CYAN, 140, 98)
SET_DRUM_TOWER_POS(TRIBE_BLACK, 252, 222)
SET_DRUM_TOWER_POS(TRIBE_ORANGE, 150, 232)
SET_MARKER_ENTRY(TRIBE_CYAN,0,37,38,0,2,2,2)
SET_MARKER_ENTRY(TRIBE_CYAN,1,39,40,0,2,2,2)
SET_MARKER_ENTRY(TRIBE_ORANGE,0,41,42,0,0,4,0)
SET_MARKER_ENTRY(TRIBE_ORANGE,2,45,46,0,2,3,3)
SET_MARKER_ENTRY(TRIBE_ORANGE,1,43,44,0,3,2,2)
SET_MARKER_ENTRY(TRIBE_BLACK,0,6,7,0,4,3,4)
SET_MARKER_ENTRY(TRIBE_BLACK,1,8,9,0,4,4,0)
SET_MARKER_ENTRY(TRIBE_BLACK,2,10,11,0,3,0,3)
SET_MARKER_ENTRY(TRIBE_BLACK,3,12,13,0,0,5,0)
SET_MARKER_ENTRY(TRIBE_BLACK,4,14,15,0,0,4,0)
SET_MARKER_ENTRY(TRIBE_BLACK,5,16,17,0,3,3,3)
SET_MARKER_ENTRY(TRIBE_BLACK,6,18,19,0,0,4,0)
SET_MARKER_ENTRY(TRIBE_BLACK,7,20,21,0,3,0,3)
SET_MARKER_ENTRY(TRIBE_BLACK,8,22,23,0,3,0,3)
SET_MARKER_ENTRY(TRIBE_BLACK,9,24,25,0,4,2,3)
...
function OnTurn()
  if (GetTurn() > 1) then
    if every2Pow(1) then
      for index,turn in ipairs(lite_attack) do
        if (GetTurn() > turn) then
          lite_attack[index] = GetTurn() + (1536 + G_RANDOM(1024))
          if (FREE_ENTRIES(availableNums[index]) > 2) then
            WRITE_CP_ATTRIB(availableNums[index], ATTR_AWAY_MEDICINE_MAN,0)
            local spells = 0
            local marker = -1
            local marker_end = -1
            if (index == 1) then
              if (GET_HEIGHT_AT_POS(29) == 0) then
                spells = M_SPELL_LAND_BRIDGE
                marker = 28
                marker_end = 30
            ...
            elseif (index == 3) then
              if (G_RANDOM(2) == 1) then
                if (GET_HEIGHT_AT_POS(32) == 0) then
                  spells = M_SPELL_LAND_BRIDGE
                  marker = 31
                  marker_end = 33
                ...
                if (GET_HEIGHT_AT_POS(35) == 0) then
                  spells = M_SPELL_LAND_BRIDGE
                  marker = 34
                  marker_end = 36
                  WRITE_CP_ATTRIB(TRIBE_ORANGE, ATTR_AWAY_MEDICINE_MAN,1)
            ...
            local troops = count_troops(availableNums[index])
            if (troops > 5) then
              if (troops > 16) then
                troops = 16
              end
              local enemy = decide_an_enemy_to_attack(availableNums[index])
              if (enemy ~= availableNums[index]) then
                ATTACK(availableNums[index],enemy,math.ceil(troops/3),1,0,200,spells,0,0,ATTACK_NORMAL,0,marker,marker_end,-1)
              end
            end
          end
        end
    ...
    if every2Pow(2) then
      for index,turn in ipairs(shaman_attack) do
        if (GetTurn() > turn) then
          ...
          if (FREE_ENTRIES(availableNums[index]) > 2) then
            if (IS_SHAMAN_AVAILABLE_FOR_ATTACK(availableNums[index]) == 1 and MANA(availableNums[index]) > 300000) then
              WRITE_CP_ATTRIB(availableNums[index],ATTR_AWAY_MEDICINE_MAN,1)
              local enemy = decide_an_enemy_to_attack(availableNums[index])
              if (index == 1 or index == 3 and GetPlayerPeople(TRIBE_BLACK) > 100) then
                enemy = TRIBE_BLACK
              end
              if (GET_NUM_ONE_OFF_SPELLS(availableNums[index],M_SPELL_ARMAGEDDON) == 0 and enemy ~= availableNums[index]) then
                ATTACK(availableNums[index],enemy,0,1,0,999,7,7,7,ATTACK_NORMAL,0,-1,-1,-1)
              else
                SPELL_ATTACK(availableNums[index],M_SPELL_ARMAGEDDON,27,0)
              end
            end
          end
        end
        ...
        if (GetTurn() > (720*15)) then
          if (GET_HEAD_TRIGGER_COUNT(64, 190) > 0) then
            if (count_people(TRIBE_BLACK,stone_head_pos,3) < 4) then
              ...
              if (GetPlayerPeople(TRIBE_BLACK) > 80 and count_troops(TRIBE_BLACK) > 20) then
                WRITE_CP_ATTRIB(TRIBE_BLACK,ATTR_AWAY_MEDICINE_MAN,0)
                ATTACK(TRIBE_BLACK,0,8,0,27,999,0,0,0,ATTACK_NORMAL,0,-1,-1,-1)
              end
            end
          end
        end
    ...
    if (every2Pow(9)) then
      ...
      if (count_troops(TRIBE_ORANGE) > 8) then
        MARKER_ENTRIES(TRIBE_ORANGE,0,1,2,-1)
      end

      if (count_troops(TRIBE_CYAN) > 8) then
        MARKER_ENTRIES(TRIBE_CYAN,0,-1,-1,-1)
        if (GET_HEIGHT_AT_POS(29) > 0) then
          MARKER_ENTRIES(TRIBE_CYAN,1,-1,-1,-1)
        end
      end
