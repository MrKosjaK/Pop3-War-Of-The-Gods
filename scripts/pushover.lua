...
function OnTurn()
  if (_gsi.Counts.GameTurn > 1) then
    if (_gsi.Counts.GameTurn > tick_c_attack) then
      tick_c_attack = _gsi.Counts.GameTurn + (1840 + G_RANDOM(512))
        ...
        if (G_RANDOM(2) == 1) then
          if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_CYAN, M_PERSON_WARRIOR) > 3) then
            if (_gsi.Players[TRIBE_CYAN].NumPeople > 30) then
              ...
              ATTACK(TRIBE_CYAN, enemy, 8+G_RANDOM(5), ATTACK_BUILDING, T_MODEL_NONE, G_RANDOM(1000), spell, M_SPELL_SWAMP, M_SPELL_SWAMP, ATTACK_NORMAL, 0, 11, mkr, 0)
            end
          end
        else
          if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_CYAN, M_PERSON_RELIGIOUS) > 4) then
            if (_gsi.Players[TRIBE_CYAN].NumPeople > 35) then
              ...
              WRITE_CP_ATTRIB(TRIBE_CYAN, ATTR_AWAY_WARRIOR, 35)
              WRITE_CP_ATTRIB(TRIBE_CYAN, ATTR_AWAY_SPY, 1)
              ATTACK(TRIBE_CYAN, enemy, 10+G_RANDOM(8), ATTACK_BUILDING, T_MODEL_NONE, G_RANDOM(1000), spell, M_SPELL_SWAMP, M_SPELL_SWAMP, ATTACK_NORMAL, 0, 8, mkr, 0)
            end
          end
        end
      end
    elseif (_gsi.Counts.GameTurn > tick_p_attack) then
      ...
      if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_PINK, M_PERSON_WARRIOR) > 5 and FREE_ENTRIES(TRIBE_PINK) > 3) then
        if (_gsi.Players[TRIBE_ORANGE].NumPeople > 0) then
          ...
          ATTACK(TRIBE_PINK, TRIBE_ORANGE, ppl+G_RANDOM(ppl), ATTACK_BUILDING, T_MODEL_NONE, G_RANDOM(1000), M_SPELL_HYPNOTISM, M_SPELL_SWAMP, M_SPELL_LIGHTNING_BOLT, ATTACK_NORMAL, 0, NO_MARKER, NO_MARKER, NO_MARKER)
        end
      end
    elseif (_gsi.Counts.GameTurn > tick_b_attack) then
      ...
      if (G_RANDOM(2) == 1 and _gsi.Players[TRIBE_BLACK].NumPeople > 45) then
        ...
        if (_gsi.Players[TRIBE_BLUE].NumPeople > 0 or _gsi.Players[TRIBE_RED].NumPeople > 0) then
          ...
          ATTACK(TRIBE_BLACK, G_RANDOM(2), troops, ATTACK_BUILDING, T_MODEL_NONE, G_RANDOM(1000), spell, M_SPELL_SWAMP, M_SPELL_WHIRLWIND, ATTACK_NORMAL, 0, 63, marker, NO_MARKER)
        elseif (_gsi.Players[TRIBE_CYAN].NumPeople > 0 or _gsi.Players[TRIBE_GREEN].NumPeople > 0) then
          if (G_RANDOM(2) == 0) then
            ...
            ATTACK(TRIBE_BLACK, enemy, troops, ATTACK_BUILDING, T_MODEL_NONE, G_RANDOM(1000), spell, M_SPELL_SWAMP, M_SPELL_WHIRLWIND, ATTACK_NORMAL, 0, marker_group, marker, NO_MARKER)
          else
            local spell = M_SPELL_WHIRLWIND
            local marker = NO_MARKER
            local marker_group = 66
        ...
        if (_gsi.Players[TRIBE_GREEN].NumPeople > 0) then
          ...
          ATTACK(TRIBE_BLACK, TRIBE_GREEN, troops, ATTACK_BUILDING, T_MODEL_NONE, G_RANDOM(1000), spell, M_SPELL_SWAMP, M_SPELL_WHIRLWIND, ATTACK_NORMAL, 0, marker_group, marker, NO_MARKER)
        end
      end
      ...
      if (_gsi.Players[TRIBE_ORANGE].NumPeople > 40 and FREE_ENTRIES(TRIBE_ORANGE) > 2) then
        if (_gsi.Players[TRIBE_PINK].NumPeople > 0 or _gsi.Players[TRIBE_CYAN].NumPeople > 0) then
          if (G_RANDOM(2) == 1) then
            ...
            ATTACK(TRIBE_ORANGE, TRIBE_PINK, 2+G_RANDOM(10), ATTACK_BUILDING, T_MODEL_NONE, G_RANDOM(200), M_SPELL_HYPNOTISM, M_SPELL_SWAMP, M_SPELL_HYPNOTISM, ATTACK_NORMAL, 0, NO_MARKER, NO_MARKER, NO_MARKER)
          else
            if (GET_HEIGHT_AT_POS(12) > 0) then
              WRITE_CP_ATTRIB(TRIBE_BLACK, ATTR_AWAY_MEDICINE_MAN, 1)
              WRITE_CP_ATTRIB(TRIBE_BLACK, ATTR_AWAY_RELIGIOUS, 25+G_RANDOM(50))
              ...
              ATTACK(TRIBE_ORANGE, TRIBE_CYAN, 10+G_RANDOM(10), ATTACK_BUILDING, T_MODEL_NONE, G_RANDOM(1000), M_SPELL_HYPNOTISM, M_SPELL_SWAMP, M_SPELL_HYPNOTISM, ATTACK_NORMAL, 0, 13, NO_MARKER, NO_MARKER)
            else
              WRITE_CP_ATTRIB(TRIBE_BLACK, ATTR_AWAY_MEDICINE_MAN, 1)
              WRITE_CP_ATTRIB(TRIBE_BLACK, ATTR_AWAY_RELIGIOUS, 25+G_RANDOM(50))
              WRITE_CP_ATTRIB(TRIBE_BLACK, ATTR_AWAY_SUPER_WARRIOR, 25+G_RANDOM(50))
    ...
    if (_gsi.Counts.GameTurn > tick_c_boat) then
      tick_c_boat = _gsi.Counts.GameTurn + (840 + G_RANDOM(320))
      if (PLAYERS_VEHICLE_OF_TYPE(TRIBE_CYAN, M_VEHICLE_BOAT_1) > 0 and FREE_ENTRIES(TRIBE_CYAN) > 1) then
        if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_CYAN, M_PERSON_SUPER_WARRIOR) > 5) then
          ...
          WRITE_CP_ATTRIB(TRIBE_CYAN, ATTR_AWAY_SPY, G_RANDOM(100))
          WRITE_CP_ATTRIB(TRIBE_CYAN, ATTR_AWAY_MEDICINE_MAN, G_RANDOM(2))
          ATTACK(TRIBE_CYAN, wta, 4+G_RANDOM(GET_NUM_OF_AVAILABLE_BOATS(TRIBE_CYAN)+1)*4, ATTACK_BUILDING, T_MODEL_NONE, 850, M_SPELL_INVISIBILITY, M_SPELL_SWAMP, M_SPELL_SWAMP, ATTACK_BY_BOAT, 0, NO_MARKER, NO_MARKER, NO_MARKER)
        end
      end
    elseif (_gsi.Counts.GameTurn > tick_b_boat) then
      tick_b_boat = _gsi.Counts.GameTurn + (1000 + G_RANDOM(380))
    ...
    if (_gsi.Counts.GameTurn > tick_p_expand) then
      tick_p_expand = _gsi.Counts.GameTurn + (480 + G_RANDOM(800))
      if (MANA(TRIBE_PINK) > 70000 and FREE_ENTRIES(TRIBE_PINK) > 2) then
        if (IS_SHAMAN_AVAILABLE_FOR_ATTACK(TRIBE_PINK) > 0) then
          if (pink_expand == 0) then
            ATTACK(TRIBE_PINK, G_RANDOM(8), 0, ATTACK_BUILDING, T_MODEL_NONE, 0, M_SPELL_LAND_BRIDGE, M_SPELL_NONE, M_SPELL_NONE, ATTACK_NORMAL, 0, 16, 17, NO_MARKER)
            pink_expand = 1
          elseif (pink_expand == 1) then
            ATTACK(TRIBE_PINK, G_RANDOM(8), 0, ATTACK_BUILDING, T_MODEL_NONE, 0, M_SPELL_LAND_BRIDGE, M_SPELL_NONE, M_SPELL_NONE, ATTACK_NORMAL, 0, 16, 18, NO_MARKER)
            pink_expand = 2
          elseif (pink_expand == 2) then
            ATTACK(TRIBE_PINK, G_RANDOM(8), 0, ATTACK_BUILDING, T_MODEL_NONE, 0, M_SPELL_LAND_BRIDGE, M_SPELL_NONE, M_SPELL_NONE, ATTACK_NORMAL, 0, 19, 20, NO_MARKER)
            pink_expand = 3
          elseif (pink_expand == 3) then
            ATTACK(TRIBE_PINK, G_RANDOM(8), 0, ATTACK_BUILDING, T_MODEL_NONE, 0, M_SPELL_LAND_BRIDGE, M_SPELL_NONE, M_SPELL_NONE, ATTACK_NORMAL, 0, 19, 21, NO_MARKER)
            pink_expand = 4
          elseif (pink_expand == 4) then
            ATTACK(TRIBE_PINK, G_RANDOM(8), 0, ATTACK_BUILDING, T_MODEL_NONE, 0, M_SPELL_LAND_BRIDGE, M_SPELL_NONE, M_SPELL_NONE, ATTACK_NORMAL, 0, 22, 23, NO_MARKER)
            pink_expand = 5
          elseif (pink_expand == 5) then
            ATTACK(TRIBE_PINK, G_RANDOM(8), 0, ATTACK_BUILDING, T_MODEL_NONE, 0, M_SPELL_LAND_BRIDGE, M_SPELL_NONE, M_SPELL_NONE, ATTACK_NORMAL, 0, 24, 25, NO_MARKER)
            pink_expand = 6
          end
        end
      end
      ...
      for i = 4,7 do
        if (PLAYERS_PEOPLE_OF_TYPE(i, M_PERSON_BRAVE) > 15 and _gsi.Players[i].NumPeople > 25) then
          ...
          if (i == TRIBE_BLACK and _gsi.Players[i].NumPeople > 80) then
            ...
            if (_gsi.Counts.GameTurn > (12*60)*8) then
