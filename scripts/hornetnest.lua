...
function OnTurn()
      ...
      if (_gsi.Players[TRIBE_BLACK].NumPeople > 45) then
        if (IS_SHAMAN_AVAILABLE_FOR_ATTACK(TRIBE_BLACK) == 1) then
          if (GET_HEIGHT_AT_POS(2) > 0) then
            WRITE_CP_ATTRIB(TRIBE_BLACK, ATTR_AWAY_MEDICINE_MAN, 100)
            ATTACK(TRIBE_BLACK, G_RANDOM(2), 6+G_RANDOM(12), ATTACK_BUILDING, T_MODEL_NONE, 250+G_RANDOM(700), M_SPELL_INSECT_PLAGUE, M_SPELL_INSECT_PLAGUE, M_SPELL_INSECT_PLAGUE, ATTACK_NORMAL, 0, 1, NO_MARKER, NO_MARKER)
          end
        end
      end
    end
  ...
  if (EVERY_2POW_TURNS(8)) then
    if (GET_HEIGHT_AT_POS(2) > 0) then
      if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_BLACK, M_PERSON_WARRIOR) > 3) then
        ...
        ATTACK(TRIBE_BLACK, G_RANDOM(2), 1+G_RANDOM(3), ATTACK_BUILDING, T_MODEL_NONE, 250+G_RANDOM(700), M_SPELL_NONE, M_SPELL_NONE, M_SPELL_NONE, ATTACK_NORMAL, 0, 1, NO_MARKER, NO_MARKER)
      end
    end
  end
  ...
  if (EVERY_2POW_TURNS(9)) then
    ...
    if (COUNT_PEOPLE_IN_MARKER(TRIBE_BLACK, TRIBE_BLUE, 8, 2) +
      ...
      WRITE_CP_ATTRIB(TRIBE_BLACK, ATTR_AWAY_MEDICINE_MAN, 100)
      ATTACK(TRIBE_BLACK, G_RANDOM(2), 1+G_RANDOM(3), ATTACK_BUILDING, T_MODEL_NONE, 250+G_RANDOM(700), M_SPELL_BLAST, M_SPELL_BLAST, M_SPELL_BLAST, ATTACK_NORMAL, 0, NO_MARKER, NO_MARKER, NO_MARKER)
    end
  end
