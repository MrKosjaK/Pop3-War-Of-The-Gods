  ...
  if (EVERY_2POW_TURNS(12)) then
    if (_gsi.Players[TRIBE_BLACK].Mana > 400000 and _gsi.Counts.GameTurn > (12*60)*9) then
      if (_gsi.Players[TRIBE_BLACK].NumPeople > 60) then
        ATTACK(TRIBE_BLACK, G_RANDOM(2), 15+G_RANDOM(15), ATTACK_BUILDING, T_MODEL_NONE, 999, M_SPELL_INVISIBILITY, M_SPELL_INVISIBILITY, M_SPELL_INVISIBILITY, ATTACK_NORMAL, 0, NO_MARKER, NO_MARKER, NO_MARKER)
      end
    end
  end
  ...
  if (_gsi.Counts.GameTurn > tickPinkAttack) then
    ...
    if (attacks == 0) then
      if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_PINK, M_PERSON_SUPER_WARRIOR) > 3 and _gsi.Players[TRIBE_PINK].NumPeople > 30) then
        ...
        ATTACK(TRIBE_PINK, G_RANDOM(2), 5+G_RANDOM(10), ATTACK_BUILDING, T_MODEL_NONE, 999, spell1, M_SPELL_LIGHTNING_BOLT, M_SPELL_LIGHTNING_BOLT, ATTACK_NORMAL, 0, 19, marker1, NO_MARKER)
      end
    elseif (attacks == 1) then
      if (PLAYERS_PEOPLE_OF_TYPE(TRIBE_PINK, M_PERSON_RELIGIOUS) > 3) then
        ...
        ATTACK(TRIBE_PINK, G_RANDOM(2), 4+G_RANDOM(9), ATTACK_BUILDING, T_MODEL_NONE, 999, spell2, M_SPELL_LIGHTNING_BOLT, M_SPELL_LIGHTNING_BOLT, ATTACK_NORMAL, 0, 27, marker2, NO_MARKER)
      end
    end
  end
